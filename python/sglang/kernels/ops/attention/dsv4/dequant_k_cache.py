...
# Triton below SM90 accepts only ('fp8e4b15', 'fp8e5') as fp8 element types, so
# a kernel whose signature carries an fp8e4nv pointer fails to compile there.
...
#
# Assembling the value with bit arithmetic instead (as the sparse decode kernel
# does, folding the E8M0 scale into the exponent) measured the same here across
# 4K-128K tokens: this kernel is bandwidth-bound, so the table gather hides
# entirely and the simpler exact decode wins.
_E4M3_LUT_CACHE = {}


def _e4m3_lut(device: torch.device) -> torch.Tensor:
    lut = _E4M3_LUT_CACHE.get(device)
    if lut is None:
        lut = (
...
def _dequantize_k_cache_paged_kernel(
    output_ptr,
    e4m3_lut_ptr,  # an fp8 pointer in the signature will not compile below SM90
    buf_bf16_ptr,
    buf_uint8_ptr,
    page_table_ptr,
    output_stride_0,

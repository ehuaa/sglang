...
class EaglePPVerifyInputRaw(SpecInput):
    ...
    def get_spec_adjust_token_coefficient(self) -> Tuple[int, int]:
        num_draft = (
            len(self.draft_tokens[0]) if self.draft_tokens and isinstance(
                self.draft_tokens[0], list
            ) else 1
        )
    ...
    def filter_batch(self, new_indices: torch.Tensor, has_been_filtered: bool = True, new_indices_cpu: torch.Tensor = None):

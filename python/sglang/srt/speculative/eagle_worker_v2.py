...
class EagleDraftWorker(EagleDraftWorkerBase):
    ...
    def init_lm_head(self):
        ...
        if pp_size > 1:
            # Under PP, the draft embedding is loaded from the draft checkpoint in
            # load_weights already; here we only take the target lm_head for the draft.
...
class EAGLEWorkerV2(BaseSpecWorker):
    ...
    def init_cuda_graphs(self):
        ...
        # Under PP, only the last rank holds the draft model; other ranks have
        # no draft worker and must skip the cuda graph init.
    ...
    def forward_batch_generation(
        ...
        # idle rank and desync the MoE all-gather -> hang. This mirrors the
        # globally-synchronized is_extend_in_batch lockstep the scheduler
        # already uses for its overlap decision (see is_disable_overlap_for_batch).
        ...
        if run_as_prefill:
            ...
            # If, PP enabled, non-last PP ranks only relay the target forward upstream.
            if self._pp_enabled and not self._pp_is_last_rank:
                return batch_output
        ...
        if self._pp_enabled and self._pp_is_last_rank:
            batch_output.pp_verify_input_raw = EaglePPVerifyInputRaw(
                ...
                bonus_tokens=batch_output.next_draft_input.bonus_tokens.to(torch.int64).tolist(),
    ...
    def verify(self, batch: ScheduleBatch, pp_proxy_tensors=None):
        return run_eagle_verify(
            ...
            pp_is_last_rank=self._pp_is_last_rank,
            pp_proxy_tensors=pp_proxy_tensors,
        )

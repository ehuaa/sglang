...
class EAGLEWorkerV2(BaseSpecWorker):
    ...
    def forward_batch_generation(
        ...
        # while the peers are idle is enough. Do not gate this on PP: at
        # pp_size=1 the DP ranks still go idle independently. The other spec
        # workers spell it the same way (`is_extend() or is_extend_in_batch` in
        # multi_layer_eagle, frozen_kv_mtp, dflash, dspark).
        #
        ...
        if self._pp_enabled:
            if batch.forward_mode.is_idle():
                ...
                # Under PP the verify input is built from the raw draft tree
                # relayed from the last PP rank of the previous iteration.

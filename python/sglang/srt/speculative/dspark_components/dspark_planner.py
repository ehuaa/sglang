    ...
    def resolve_verify_token_budget(
        ...
        *,
        ...
        if not self.server_args.disable_overlap_schedule:
            assert isinstance(
                draft_input, DFlashDraftInputV2
            ), "overlap path reads draft_input.verify_token_budget; expected DFlashDraftInputV2, not PP raw"
...
def alloc_verify_window(
    *,
    ...
    # Allow callers to pass post-iter seq_lens explicitly. Defaults to the
    # batch's current seq_lens so behavior is unchanged for existing callers.

...
class DSparkPPVerifyInputRaw(DFlashDecodePrepareMixin, SpecInput):
    ...
    def __post_init__(self):
        super().__init__(SpecInputType.DSPARK_PP_VERIFY_INPUT_RAW)
    ...
    def filter_batch(
        ...
        # Prefer the caller-provided CPU indices when available to avoid a
        # device->cpu copy; fall back to converting new_indices.
        if new_indices_cpu is not None:
            idx = list(new_indices_cpu)
        else:
            idx = (

...
class DSparkWorkerV2(BaseSpecWorker):
    ...
    def __init__(
        ...
        # PP state: the draft model lives only on the last PP rank; non-last
        # ranks relay the target verify forward and early-return afterwards.
    ...
    def _forward_decode(
        ...
        proposal = None
        # Candidate source: single-rank proposes this iter's drafts; under PP
        # every rank rebuilds candidates from the last rank's relayed raw.
        if pp_raw is None:
        ...
        if pp_raw is not None:
            logger.warning(
                "[DEBUG PP-DSpark] pp_raw.draft_tokens=%s pp_raw.bonus_tokens=%s "
                "pp_raw.new_seq_lens=%s gamma=%s",
                pp_raw.draft_tokens,
                pp_raw.bonus_tokens,
                pp_raw.new_seq_lens,
                getattr(self, "gamma", "N/A"),
            )
        ...
        # PP last rank: re-propose to produce next iter's candidates and pack
        # the relay raw (draft + confidence + accounting) for all verify ranks.
        # The relayed draft tokens are verified by other PP ranks at the next
        # iter, where positions are computed from post-iter seq_lens. Reusing
        # the pre-iter verify_window here would make the relayed draft use
        # positions offset by commit_lens, causing PP hidden misalignment.
        # Build a post-iter verify_window so propose uses the same positions
        # the non-last ranks will use to verify.
        pp_raw_out = None
        if self._pp_enabled:
